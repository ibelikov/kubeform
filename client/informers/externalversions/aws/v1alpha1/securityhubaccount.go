...
// SecurityhubAccountInformer provides access to a shared informer and lister for
// SecurityhubAccounts.
type SecurityhubAccountInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.SecurityhubAccountLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().SecurityhubAccounts(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().SecurityhubAccounts(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *securityhubAccountInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.SecurityhubAccount{}, f.defaultInformer)
}

func (f *securityhubAccountInformer) Lister() v1alpha1.SecurityhubAccountLister {
	return v1alpha1.NewSecurityhubAccountLister(f.Informer().GetIndexer())
}
