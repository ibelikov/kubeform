...
// BudgetsBudgetInformer provides access to a shared informer and lister for
// BudgetsBudgets.
type BudgetsBudgetInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.BudgetsBudgetLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().BudgetsBudgets(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().BudgetsBudgets(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *budgetsBudgetInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.BudgetsBudget{}, f.defaultInformer)
}

func (f *budgetsBudgetInformer) Lister() v1alpha1.BudgetsBudgetLister {
	return v1alpha1.NewBudgetsBudgetLister(f.Informer().GetIndexer())
}
