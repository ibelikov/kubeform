...
// StoragegatewayUploadBufferInformer provides access to a shared informer and lister for
// StoragegatewayUploadBuffers.
type StoragegatewayUploadBufferInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.StoragegatewayUploadBufferLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().StoragegatewayUploadBuffers(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().StoragegatewayUploadBuffers(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *storagegatewayUploadBufferInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.StoragegatewayUploadBuffer{}, f.defaultInformer)
}

func (f *storagegatewayUploadBufferInformer) Lister() v1alpha1.StoragegatewayUploadBufferLister {
	return v1alpha1.NewStoragegatewayUploadBufferLister(f.Informer().GetIndexer())
}
