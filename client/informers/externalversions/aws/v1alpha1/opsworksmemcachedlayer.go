...
// OpsworksMemcachedLayerInformer provides access to a shared informer and lister for
// OpsworksMemcachedLayers.
type OpsworksMemcachedLayerInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.OpsworksMemcachedLayerLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().OpsworksMemcachedLayers(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().OpsworksMemcachedLayers(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *opsworksMemcachedLayerInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.OpsworksMemcachedLayer{}, f.defaultInformer)
}

func (f *opsworksMemcachedLayerInformer) Lister() v1alpha1.OpsworksMemcachedLayerLister {
	return v1alpha1.NewOpsworksMemcachedLayerLister(f.Informer().GetIndexer())
}
