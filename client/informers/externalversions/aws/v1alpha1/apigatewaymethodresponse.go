...
// ApiGatewayMethodResponseInformer provides access to a shared informer and lister for
// ApiGatewayMethodResponses.
type ApiGatewayMethodResponseInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.ApiGatewayMethodResponseLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().ApiGatewayMethodResponses(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().ApiGatewayMethodResponses(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *apiGatewayMethodResponseInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.ApiGatewayMethodResponse{}, f.defaultInformer)
}

func (f *apiGatewayMethodResponseInformer) Lister() v1alpha1.ApiGatewayMethodResponseLister {
	return v1alpha1.NewApiGatewayMethodResponseLister(f.Informer().GetIndexer())
}
