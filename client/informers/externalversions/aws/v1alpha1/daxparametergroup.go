...
// DaxParameterGroupInformer provides access to a shared informer and lister for
// DaxParameterGroups.
type DaxParameterGroupInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.DaxParameterGroupLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().DaxParameterGroups(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().DaxParameterGroups(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *daxParameterGroupInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.DaxParameterGroup{}, f.defaultInformer)
}

func (f *daxParameterGroupInformer) Lister() v1alpha1.DaxParameterGroupLister {
	return v1alpha1.NewDaxParameterGroupLister(f.Informer().GetIndexer())
}
