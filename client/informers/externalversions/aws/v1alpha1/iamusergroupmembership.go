...
// IamUserGroupMembershipInformer provides access to a shared informer and lister for
// IamUserGroupMemberships.
type IamUserGroupMembershipInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.IamUserGroupMembershipLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().IamUserGroupMemberships(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().IamUserGroupMemberships(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *iamUserGroupMembershipInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.IamUserGroupMembership{}, f.defaultInformer)
}

func (f *iamUserGroupMembershipInformer) Lister() v1alpha1.IamUserGroupMembershipLister {
	return v1alpha1.NewIamUserGroupMembershipLister(f.Informer().GetIndexer())
}
