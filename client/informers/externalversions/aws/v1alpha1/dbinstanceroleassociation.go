...
// DbInstanceRoleAssociationInformer provides access to a shared informer and lister for
// DbInstanceRoleAssociations.
type DbInstanceRoleAssociationInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.DbInstanceRoleAssociationLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().DbInstanceRoleAssociations(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().DbInstanceRoleAssociations(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *dbInstanceRoleAssociationInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.DbInstanceRoleAssociation{}, f.defaultInformer)
}

func (f *dbInstanceRoleAssociationInformer) Lister() v1alpha1.DbInstanceRoleAssociationLister {
	return v1alpha1.NewDbInstanceRoleAssociationLister(f.Informer().GetIndexer())
}
