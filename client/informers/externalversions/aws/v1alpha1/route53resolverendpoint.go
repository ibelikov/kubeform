...
// Route53ResolverEndpointInformer provides access to a shared informer and lister for
// Route53ResolverEndpoints.
type Route53ResolverEndpointInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.Route53ResolverEndpointLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().Route53ResolverEndpoints(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().Route53ResolverEndpoints(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *route53ResolverEndpointInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.Route53ResolverEndpoint{}, f.defaultInformer)
}

func (f *route53ResolverEndpointInformer) Lister() v1alpha1.Route53ResolverEndpointLister {
	return v1alpha1.NewRoute53ResolverEndpointLister(f.Informer().GetIndexer())
}
