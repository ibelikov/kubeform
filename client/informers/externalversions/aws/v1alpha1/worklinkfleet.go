...
// WorklinkFleetInformer provides access to a shared informer and lister for
// WorklinkFleets.
type WorklinkFleetInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.WorklinkFleetLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().WorklinkFleets(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().WorklinkFleets(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *worklinkFleetInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.WorklinkFleet{}, f.defaultInformer)
}

func (f *worklinkFleetInformer) Lister() v1alpha1.WorklinkFleetLister {
	return v1alpha1.NewWorklinkFleetLister(f.Informer().GetIndexer())
}
