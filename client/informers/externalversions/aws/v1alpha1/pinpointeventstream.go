...
// PinpointEventStreamInformer provides access to a shared informer and lister for
// PinpointEventStreams.
type PinpointEventStreamInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.PinpointEventStreamLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().PinpointEventStreams(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().PinpointEventStreams(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *pinpointEventStreamInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.PinpointEventStream{}, f.defaultInformer)
}

func (f *pinpointEventStreamInformer) Lister() v1alpha1.PinpointEventStreamLister {
	return v1alpha1.NewPinpointEventStreamLister(f.Informer().GetIndexer())
}
