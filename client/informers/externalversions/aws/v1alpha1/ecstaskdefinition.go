...
// EcsTaskDefinitionInformer provides access to a shared informer and lister for
// EcsTaskDefinitions.
type EcsTaskDefinitionInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.EcsTaskDefinitionLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().EcsTaskDefinitions(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().EcsTaskDefinitions(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *ecsTaskDefinitionInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.EcsTaskDefinition{}, f.defaultInformer)
}

func (f *ecsTaskDefinitionInformer) Lister() v1alpha1.EcsTaskDefinitionLister {
	return v1alpha1.NewEcsTaskDefinitionLister(f.Informer().GetIndexer())
}
