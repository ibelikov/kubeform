...
// LambdaAliasInformer provides access to a shared informer and lister for
// LambdaAliases.
type LambdaAliasInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.LambdaAliasLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().LambdaAliases(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().LambdaAliases(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *lambdaAliasInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.LambdaAlias{}, f.defaultInformer)
}

func (f *lambdaAliasInformer) Lister() v1alpha1.LambdaAliasLister {
	return v1alpha1.NewLambdaAliasLister(f.Informer().GetIndexer())
}
