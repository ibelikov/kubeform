...
// IamRolePolicyAttachmentInformer provides access to a shared informer and lister for
// IamRolePolicyAttachments.
type IamRolePolicyAttachmentInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.IamRolePolicyAttachmentLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().IamRolePolicyAttachments(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().IamRolePolicyAttachments(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *iamRolePolicyAttachmentInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.IamRolePolicyAttachment{}, f.defaultInformer)
}

func (f *iamRolePolicyAttachmentInformer) Lister() v1alpha1.IamRolePolicyAttachmentLister {
	return v1alpha1.NewIamRolePolicyAttachmentLister(f.Informer().GetIndexer())
}
