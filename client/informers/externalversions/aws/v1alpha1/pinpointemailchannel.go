...
// PinpointEmailChannelInformer provides access to a shared informer and lister for
// PinpointEmailChannels.
type PinpointEmailChannelInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.PinpointEmailChannelLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().PinpointEmailChannels(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().PinpointEmailChannels(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *pinpointEmailChannelInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.PinpointEmailChannel{}, f.defaultInformer)
}

func (f *pinpointEmailChannelInformer) Lister() v1alpha1.PinpointEmailChannelLister {
	return v1alpha1.NewPinpointEmailChannelLister(f.Informer().GetIndexer())
}
