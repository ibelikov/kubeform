...
// SpotInstanceRequestInformer provides access to a shared informer and lister for
// SpotInstanceRequests.
type SpotInstanceRequestInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.SpotInstanceRequestLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().SpotInstanceRequests(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().SpotInstanceRequests(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *spotInstanceRequestInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.SpotInstanceRequest{}, f.defaultInformer)
}

func (f *spotInstanceRequestInformer) Lister() v1alpha1.SpotInstanceRequestLister {
	return v1alpha1.NewSpotInstanceRequestLister(f.Informer().GetIndexer())
}
