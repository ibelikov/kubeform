...
// AutoscalingNotificationInformer provides access to a shared informer and lister for
// AutoscalingNotifications.
type AutoscalingNotificationInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.AutoscalingNotificationLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().AutoscalingNotifications(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().AutoscalingNotifications(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *autoscalingNotificationInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.AutoscalingNotification{}, f.defaultInformer)
}

func (f *autoscalingNotificationInformer) Lister() v1alpha1.AutoscalingNotificationLister {
	return v1alpha1.NewAutoscalingNotificationLister(f.Informer().GetIndexer())
}
