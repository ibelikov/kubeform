...
// DefaultVpcInformer provides access to a shared informer and lister for
// DefaultVpcs.
type DefaultVpcInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.DefaultVpcLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().DefaultVpcs(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().DefaultVpcs(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *defaultVpcInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.DefaultVpc{}, f.defaultInformer)
}

func (f *defaultVpcInformer) Lister() v1alpha1.DefaultVpcLister {
	return v1alpha1.NewDefaultVpcLister(f.Informer().GetIndexer())
}
