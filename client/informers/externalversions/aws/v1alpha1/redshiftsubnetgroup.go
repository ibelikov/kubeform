...
// RedshiftSubnetGroupInformer provides access to a shared informer and lister for
// RedshiftSubnetGroups.
type RedshiftSubnetGroupInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.RedshiftSubnetGroupLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().RedshiftSubnetGroups(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().RedshiftSubnetGroups(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *redshiftSubnetGroupInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.RedshiftSubnetGroup{}, f.defaultInformer)
}

func (f *redshiftSubnetGroupInformer) Lister() v1alpha1.RedshiftSubnetGroupLister {
	return v1alpha1.NewRedshiftSubnetGroupLister(f.Informer().GetIndexer())
}
