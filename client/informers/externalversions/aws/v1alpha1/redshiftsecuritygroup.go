...
// RedshiftSecurityGroupInformer provides access to a shared informer and lister for
// RedshiftSecurityGroups.
type RedshiftSecurityGroupInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.RedshiftSecurityGroupLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().RedshiftSecurityGroups(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().RedshiftSecurityGroups(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *redshiftSecurityGroupInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.RedshiftSecurityGroup{}, f.defaultInformer)
}

func (f *redshiftSecurityGroupInformer) Lister() v1alpha1.RedshiftSecurityGroupLister {
	return v1alpha1.NewRedshiftSecurityGroupLister(f.Informer().GetIndexer())
}
