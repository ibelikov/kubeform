...
// WafregionalByteMatchSetInformer provides access to a shared informer and lister for
// WafregionalByteMatchSets.
type WafregionalByteMatchSetInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.WafregionalByteMatchSetLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().WafregionalByteMatchSets(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().WafregionalByteMatchSets(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *wafregionalByteMatchSetInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.WafregionalByteMatchSet{}, f.defaultInformer)
}

func (f *wafregionalByteMatchSetInformer) Lister() v1alpha1.WafregionalByteMatchSetLister {
	return v1alpha1.NewWafregionalByteMatchSetLister(f.Informer().GetIndexer())
}
