...
// PinpointApnsSandboxChannelInformer provides access to a shared informer and lister for
// PinpointApnsSandboxChannels.
type PinpointApnsSandboxChannelInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.PinpointApnsSandboxChannelLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().PinpointApnsSandboxChannels(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().PinpointApnsSandboxChannels(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *pinpointApnsSandboxChannelInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.PinpointApnsSandboxChannel{}, f.defaultInformer)
}

func (f *pinpointApnsSandboxChannelInformer) Lister() v1alpha1.PinpointApnsSandboxChannelLister {
	return v1alpha1.NewPinpointApnsSandboxChannelLister(f.Informer().GetIndexer())
}
