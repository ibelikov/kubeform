...
// CloudhsmV2ClusterInformer provides access to a shared informer and lister for
// CloudhsmV2Clusters.
type CloudhsmV2ClusterInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.CloudhsmV2ClusterLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().CloudhsmV2Clusters(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().CloudhsmV2Clusters(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *cloudhsmV2ClusterInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.CloudhsmV2Cluster{}, f.defaultInformer)
}

func (f *cloudhsmV2ClusterInformer) Lister() v1alpha1.CloudhsmV2ClusterLister {
	return v1alpha1.NewCloudhsmV2ClusterLister(f.Informer().GetIndexer())
}
