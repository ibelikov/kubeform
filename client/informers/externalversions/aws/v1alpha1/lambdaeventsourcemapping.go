...
// LambdaEventSourceMappingInformer provides access to a shared informer and lister for
// LambdaEventSourceMappings.
type LambdaEventSourceMappingInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.LambdaEventSourceMappingLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().LambdaEventSourceMappings(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().LambdaEventSourceMappings(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *lambdaEventSourceMappingInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.LambdaEventSourceMapping{}, f.defaultInformer)
}

func (f *lambdaEventSourceMappingInformer) Lister() v1alpha1.LambdaEventSourceMappingLister {
	return v1alpha1.NewLambdaEventSourceMappingLister(f.Informer().GetIndexer())
}
