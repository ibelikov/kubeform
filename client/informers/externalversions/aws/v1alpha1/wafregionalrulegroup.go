...
// WafregionalRuleGroupInformer provides access to a shared informer and lister for
// WafregionalRuleGroups.
type WafregionalRuleGroupInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.WafregionalRuleGroupLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().WafregionalRuleGroups(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().WafregionalRuleGroups(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *wafregionalRuleGroupInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.WafregionalRuleGroup{}, f.defaultInformer)
}

func (f *wafregionalRuleGroupInformer) Lister() v1alpha1.WafregionalRuleGroupLister {
	return v1alpha1.NewWafregionalRuleGroupLister(f.Informer().GetIndexer())
}
