...
// InspectorAssessmentTargetInformer provides access to a shared informer and lister for
// InspectorAssessmentTargets.
type InspectorAssessmentTargetInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.InspectorAssessmentTargetLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().InspectorAssessmentTargets(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().InspectorAssessmentTargets(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *inspectorAssessmentTargetInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.InspectorAssessmentTarget{}, f.defaultInformer)
}

func (f *inspectorAssessmentTargetInformer) Lister() v1alpha1.InspectorAssessmentTargetLister {
	return v1alpha1.NewInspectorAssessmentTargetLister(f.Informer().GetIndexer())
}
