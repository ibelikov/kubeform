...
// GuarddutyMemberInformer provides access to a shared informer and lister for
// GuarddutyMembers.
type GuarddutyMemberInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.GuarddutyMemberLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().GuarddutyMembers(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().GuarddutyMembers(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *guarddutyMemberInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.GuarddutyMember{}, f.defaultInformer)
}

func (f *guarddutyMemberInformer) Lister() v1alpha1.GuarddutyMemberLister {
	return v1alpha1.NewGuarddutyMemberLister(f.Informer().GetIndexer())
}
