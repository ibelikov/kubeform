...
// DocdbSubnetGroupInformer provides access to a shared informer and lister for
// DocdbSubnetGroups.
type DocdbSubnetGroupInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.DocdbSubnetGroupLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().DocdbSubnetGroups(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().DocdbSubnetGroups(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *docdbSubnetGroupInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.DocdbSubnetGroup{}, f.defaultInformer)
}

func (f *docdbSubnetGroupInformer) Lister() v1alpha1.DocdbSubnetGroupLister {
	return v1alpha1.NewDocdbSubnetGroupLister(f.Informer().GetIndexer())
}
