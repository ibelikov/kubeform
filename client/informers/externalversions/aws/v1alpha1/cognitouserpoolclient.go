...
// CognitoUserPoolClientInformer provides access to a shared informer and lister for
// CognitoUserPoolClients.
type CognitoUserPoolClientInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.CognitoUserPoolClientLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().CognitoUserPoolClients(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().CognitoUserPoolClients(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *cognitoUserPoolClientInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.CognitoUserPoolClient{}, f.defaultInformer)
}

func (f *cognitoUserPoolClientInformer) Lister() v1alpha1.CognitoUserPoolClientLister {
	return v1alpha1.NewCognitoUserPoolClientLister(f.Informer().GetIndexer())
}
