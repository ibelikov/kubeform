...
// LambdaLayerVersionInformer provides access to a shared informer and lister for
// LambdaLayerVersions.
type LambdaLayerVersionInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.LambdaLayerVersionLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().LambdaLayerVersions(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().LambdaLayerVersions(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *lambdaLayerVersionInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.LambdaLayerVersion{}, f.defaultInformer)
}

func (f *lambdaLayerVersionInformer) Lister() v1alpha1.LambdaLayerVersionLister {
	return v1alpha1.NewLambdaLayerVersionLister(f.Informer().GetIndexer())
}
