...
// IotPolicyAttachmentInformer provides access to a shared informer and lister for
// IotPolicyAttachments.
type IotPolicyAttachmentInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.IotPolicyAttachmentLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().IotPolicyAttachments(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().IotPolicyAttachments(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *iotPolicyAttachmentInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.IotPolicyAttachment{}, f.defaultInformer)
}

func (f *iotPolicyAttachmentInformer) Lister() v1alpha1.IotPolicyAttachmentLister {
	return v1alpha1.NewIotPolicyAttachmentLister(f.Informer().GetIndexer())
}
