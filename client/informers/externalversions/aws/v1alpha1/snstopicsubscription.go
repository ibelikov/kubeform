...
// SnsTopicSubscriptionInformer provides access to a shared informer and lister for
// SnsTopicSubscriptions.
type SnsTopicSubscriptionInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.SnsTopicSubscriptionLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().SnsTopicSubscriptions(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().SnsTopicSubscriptions(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *snsTopicSubscriptionInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.SnsTopicSubscription{}, f.defaultInformer)
}

func (f *snsTopicSubscriptionInformer) Lister() v1alpha1.SnsTopicSubscriptionLister {
	return v1alpha1.NewSnsTopicSubscriptionLister(f.Informer().GetIndexer())
}
