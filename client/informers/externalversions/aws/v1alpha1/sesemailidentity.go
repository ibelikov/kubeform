...
// SesEmailIdentityInformer provides access to a shared informer and lister for
// SesEmailIdentities.
type SesEmailIdentityInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.SesEmailIdentityLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().SesEmailIdentities(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().SesEmailIdentities(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *sesEmailIdentityInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.SesEmailIdentity{}, f.defaultInformer)
}

func (f *sesEmailIdentityInformer) Lister() v1alpha1.SesEmailIdentityLister {
	return v1alpha1.NewSesEmailIdentityLister(f.Informer().GetIndexer())
}
