...
// ElasticacheClusterInformer provides access to a shared informer and lister for
// ElasticacheClusters.
type ElasticacheClusterInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.ElasticacheClusterLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().ElasticacheClusters(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().ElasticacheClusters(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *elasticacheClusterInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.ElasticacheCluster{}, f.defaultInformer)
}

func (f *elasticacheClusterInformer) Lister() v1alpha1.ElasticacheClusterLister {
	return v1alpha1.NewElasticacheClusterLister(f.Informer().GetIndexer())
}
