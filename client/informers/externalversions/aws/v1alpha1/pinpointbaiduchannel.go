...
// PinpointBaiduChannelInformer provides access to a shared informer and lister for
// PinpointBaiduChannels.
type PinpointBaiduChannelInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.PinpointBaiduChannelLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().PinpointBaiduChannels(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().PinpointBaiduChannels(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *pinpointBaiduChannelInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.PinpointBaiduChannel{}, f.defaultInformer)
}

func (f *pinpointBaiduChannelInformer) Lister() v1alpha1.PinpointBaiduChannelLister {
	return v1alpha1.NewPinpointBaiduChannelLister(f.Informer().GetIndexer())
}
