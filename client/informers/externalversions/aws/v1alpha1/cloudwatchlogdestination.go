...
// CloudwatchLogDestinationInformer provides access to a shared informer and lister for
// CloudwatchLogDestinations.
type CloudwatchLogDestinationInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.CloudwatchLogDestinationLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().CloudwatchLogDestinations(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().CloudwatchLogDestinations(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *cloudwatchLogDestinationInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.CloudwatchLogDestination{}, f.defaultInformer)
}

func (f *cloudwatchLogDestinationInformer) Lister() v1alpha1.CloudwatchLogDestinationLister {
	return v1alpha1.NewCloudwatchLogDestinationLister(f.Informer().GetIndexer())
}
