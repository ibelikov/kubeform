...
// CodedeployDeploymentConfigInformer provides access to a shared informer and lister for
// CodedeployDeploymentConfigs.
type CodedeployDeploymentConfigInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.CodedeployDeploymentConfigLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().CodedeployDeploymentConfigs(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().CodedeployDeploymentConfigs(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *codedeployDeploymentConfigInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.CodedeployDeploymentConfig{}, f.defaultInformer)
}

func (f *codedeployDeploymentConfigInformer) Lister() v1alpha1.CodedeployDeploymentConfigLister {
	return v1alpha1.NewCodedeployDeploymentConfigLister(f.Informer().GetIndexer())
}
