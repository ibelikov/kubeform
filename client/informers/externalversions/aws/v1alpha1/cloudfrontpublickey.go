...
// CloudfrontPublicKeyInformer provides access to a shared informer and lister for
// CloudfrontPublicKeys.
type CloudfrontPublicKeyInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.CloudfrontPublicKeyLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AwsV1alpha1().CloudfrontPublicKeys(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AwsV1alpha1().CloudfrontPublicKeys(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *cloudfrontPublicKeyInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&awsv1alpha1.CloudfrontPublicKey{}, f.defaultInformer)
}

func (f *cloudfrontPublicKeyInformer) Lister() v1alpha1.CloudfrontPublicKeyLister {
	return v1alpha1.NewCloudfrontPublicKeyLister(f.Informer().GetIndexer())
}
