...
// EventhubConsumerGroupInformer provides access to a shared informer and lister for
// EventhubConsumerGroups.
type EventhubConsumerGroupInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.EventhubConsumerGroupLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().EventhubConsumerGroups(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().EventhubConsumerGroups(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *eventhubConsumerGroupInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.EventhubConsumerGroup{}, f.defaultInformer)
}

func (f *eventhubConsumerGroupInformer) Lister() v1alpha1.EventhubConsumerGroupLister {
	return v1alpha1.NewEventhubConsumerGroupLister(f.Informer().GetIndexer())
}
