...
// ApiManagementProductGroupInformer provides access to a shared informer and lister for
// ApiManagementProductGroups.
type ApiManagementProductGroupInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.ApiManagementProductGroupLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().ApiManagementProductGroups(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().ApiManagementProductGroups(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *apiManagementProductGroupInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.ApiManagementProductGroup{}, f.defaultInformer)
}

func (f *apiManagementProductGroupInformer) Lister() v1alpha1.ApiManagementProductGroupLister {
	return v1alpha1.NewApiManagementProductGroupLister(f.Informer().GetIndexer())
}
