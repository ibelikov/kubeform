...
// CosmosdbMongoCollectionInformer provides access to a shared informer and lister for
// CosmosdbMongoCollections.
type CosmosdbMongoCollectionInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.CosmosdbMongoCollectionLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().CosmosdbMongoCollections(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().CosmosdbMongoCollections(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *cosmosdbMongoCollectionInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.CosmosdbMongoCollection{}, f.defaultInformer)
}

func (f *cosmosdbMongoCollectionInformer) Lister() v1alpha1.CosmosdbMongoCollectionLister {
	return v1alpha1.NewCosmosdbMongoCollectionLister(f.Informer().GetIndexer())
}
