...
// EventhubAuthorizationRuleInformer provides access to a shared informer and lister for
// EventhubAuthorizationRules.
type EventhubAuthorizationRuleInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.EventhubAuthorizationRuleLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().EventhubAuthorizationRules(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().EventhubAuthorizationRules(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *eventhubAuthorizationRuleInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.EventhubAuthorizationRule{}, f.defaultInformer)
}

func (f *eventhubAuthorizationRuleInformer) Lister() v1alpha1.EventhubAuthorizationRuleLister {
	return v1alpha1.NewEventhubAuthorizationRuleLister(f.Informer().GetIndexer())
}
