...
// NetworkPacketCaptureInformer provides access to a shared informer and lister for
// NetworkPacketCaptures.
type NetworkPacketCaptureInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.NetworkPacketCaptureLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().NetworkPacketCaptures(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().NetworkPacketCaptures(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *networkPacketCaptureInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.NetworkPacketCapture{}, f.defaultInformer)
}

func (f *networkPacketCaptureInformer) Lister() v1alpha1.NetworkPacketCaptureLister {
	return v1alpha1.NewNetworkPacketCaptureLister(f.Informer().GetIndexer())
}
