...
// AzureadServicePrincipalInformer provides access to a shared informer and lister for
// AzureadServicePrincipals.
type AzureadServicePrincipalInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.AzureadServicePrincipalLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().AzureadServicePrincipals(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().AzureadServicePrincipals(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *azureadServicePrincipalInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.AzureadServicePrincipal{}, f.defaultInformer)
}

func (f *azureadServicePrincipalInformer) Lister() v1alpha1.AzureadServicePrincipalLister {
	return v1alpha1.NewAzureadServicePrincipalLister(f.Informer().GetIndexer())
}
