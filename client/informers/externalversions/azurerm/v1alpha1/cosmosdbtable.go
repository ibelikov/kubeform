...
// CosmosdbTableInformer provides access to a shared informer and lister for
// CosmosdbTables.
type CosmosdbTableInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.CosmosdbTableLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().CosmosdbTables(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().CosmosdbTables(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *cosmosdbTableInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.CosmosdbTable{}, f.defaultInformer)
}

func (f *cosmosdbTableInformer) Lister() v1alpha1.CosmosdbTableLister {
	return v1alpha1.NewCosmosdbTableLister(f.Informer().GetIndexer())
}
