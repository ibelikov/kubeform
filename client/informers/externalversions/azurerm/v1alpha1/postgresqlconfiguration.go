...
// PostgresqlConfigurationInformer provides access to a shared informer and lister for
// PostgresqlConfigurations.
type PostgresqlConfigurationInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.PostgresqlConfigurationLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().PostgresqlConfigurations(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().PostgresqlConfigurations(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *postgresqlConfigurationInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.PostgresqlConfiguration{}, f.defaultInformer)
}

func (f *postgresqlConfigurationInformer) Lister() v1alpha1.PostgresqlConfigurationLister {
	return v1alpha1.NewPostgresqlConfigurationLister(f.Informer().GetIndexer())
}
