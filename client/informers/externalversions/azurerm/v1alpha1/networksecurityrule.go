...
// NetworkSecurityRuleInformer provides access to a shared informer and lister for
// NetworkSecurityRules.
type NetworkSecurityRuleInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.NetworkSecurityRuleLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().NetworkSecurityRules(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().NetworkSecurityRules(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *networkSecurityRuleInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.NetworkSecurityRule{}, f.defaultInformer)
}

func (f *networkSecurityRuleInformer) Lister() v1alpha1.NetworkSecurityRuleLister {
	return v1alpha1.NewNetworkSecurityRuleLister(f.Informer().GetIndexer())
}
