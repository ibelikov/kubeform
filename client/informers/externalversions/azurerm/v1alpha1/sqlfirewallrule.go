...
// SqlFirewallRuleInformer provides access to a shared informer and lister for
// SqlFirewallRules.
type SqlFirewallRuleInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.SqlFirewallRuleLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().SqlFirewallRules(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().SqlFirewallRules(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *sqlFirewallRuleInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.SqlFirewallRule{}, f.defaultInformer)
}

func (f *sqlFirewallRuleInformer) Lister() v1alpha1.SqlFirewallRuleLister {
	return v1alpha1.NewSqlFirewallRuleLister(f.Informer().GetIndexer())
}
