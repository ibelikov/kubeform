...
// DataFactoryPipelineInformer provides access to a shared informer and lister for
// DataFactoryPipelines.
type DataFactoryPipelineInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.DataFactoryPipelineLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().DataFactoryPipelines(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().DataFactoryPipelines(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *dataFactoryPipelineInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.DataFactoryPipeline{}, f.defaultInformer)
}

func (f *dataFactoryPipelineInformer) Lister() v1alpha1.DataFactoryPipelineLister {
	return v1alpha1.NewDataFactoryPipelineLister(f.Informer().GetIndexer())
}
