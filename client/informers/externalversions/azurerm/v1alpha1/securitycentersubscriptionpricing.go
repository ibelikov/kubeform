...
// SecurityCenterSubscriptionPricingInformer provides access to a shared informer and lister for
// SecurityCenterSubscriptionPricings.
type SecurityCenterSubscriptionPricingInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.SecurityCenterSubscriptionPricingLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().SecurityCenterSubscriptionPricings(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().SecurityCenterSubscriptionPricings(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *securityCenterSubscriptionPricingInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.SecurityCenterSubscriptionPricing{}, f.defaultInformer)
}

func (f *securityCenterSubscriptionPricingInformer) Lister() v1alpha1.SecurityCenterSubscriptionPricingLister {
	return v1alpha1.NewSecurityCenterSubscriptionPricingLister(f.Informer().GetIndexer())
}
