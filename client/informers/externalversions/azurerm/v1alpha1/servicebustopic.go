...
// ServicebusTopicInformer provides access to a shared informer and lister for
// ServicebusTopics.
type ServicebusTopicInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.ServicebusTopicLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().ServicebusTopics(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().ServicebusTopics(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *servicebusTopicInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.ServicebusTopic{}, f.defaultInformer)
}

func (f *servicebusTopicInformer) Lister() v1alpha1.ServicebusTopicLister {
	return v1alpha1.NewServicebusTopicLister(f.Informer().GetIndexer())
}
