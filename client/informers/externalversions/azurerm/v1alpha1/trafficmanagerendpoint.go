...
// TrafficManagerEndpointInformer provides access to a shared informer and lister for
// TrafficManagerEndpoints.
type TrafficManagerEndpointInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.TrafficManagerEndpointLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().TrafficManagerEndpoints(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().TrafficManagerEndpoints(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *trafficManagerEndpointInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.TrafficManagerEndpoint{}, f.defaultInformer)
}

func (f *trafficManagerEndpointInformer) Lister() v1alpha1.TrafficManagerEndpointLister {
	return v1alpha1.NewTrafficManagerEndpointLister(f.Informer().GetIndexer())
}
