...
// DataLakeStoreFileInformer provides access to a shared informer and lister for
// DataLakeStoreFiles.
type DataLakeStoreFileInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.DataLakeStoreFileLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().DataLakeStoreFiles(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().DataLakeStoreFiles(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *dataLakeStoreFileInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.DataLakeStoreFile{}, f.defaultInformer)
}

func (f *dataLakeStoreFileInformer) Lister() v1alpha1.DataLakeStoreFileLister {
	return v1alpha1.NewDataLakeStoreFileLister(f.Informer().GetIndexer())
}
