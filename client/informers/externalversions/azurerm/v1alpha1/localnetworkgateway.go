...
// LocalNetworkGatewayInformer provides access to a shared informer and lister for
// LocalNetworkGateways.
type LocalNetworkGatewayInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.LocalNetworkGatewayLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().LocalNetworkGateways(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().LocalNetworkGateways(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *localNetworkGatewayInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.LocalNetworkGateway{}, f.defaultInformer)
}

func (f *localNetworkGatewayInformer) Lister() v1alpha1.LocalNetworkGatewayLister {
	return v1alpha1.NewLocalNetworkGatewayLister(f.Informer().GetIndexer())
}
