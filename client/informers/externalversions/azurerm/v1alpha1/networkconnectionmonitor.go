...
// NetworkConnectionMonitorInformer provides access to a shared informer and lister for
// NetworkConnectionMonitors.
type NetworkConnectionMonitorInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.NetworkConnectionMonitorLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().NetworkConnectionMonitors(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().NetworkConnectionMonitors(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *networkConnectionMonitorInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.NetworkConnectionMonitor{}, f.defaultInformer)
}

func (f *networkConnectionMonitorInformer) Lister() v1alpha1.NetworkConnectionMonitorLister {
	return v1alpha1.NewNetworkConnectionMonitorLister(f.Informer().GetIndexer())
}
