...
// AppServiceSlotInformer provides access to a shared informer and lister for
// AppServiceSlots.
type AppServiceSlotInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1alpha1.AppServiceSlotLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.AzurermV1alpha1().AppServiceSlots(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.AzurermV1alpha1().AppServiceSlots(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *appServiceSlotInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&azurermv1alpha1.AppServiceSlot{}, f.defaultInformer)
}

func (f *appServiceSlotInformer) Lister() v1alpha1.AppServiceSlotLister {
	return v1alpha1.NewAppServiceSlotLister(f.Informer().GetIndexer())
}
