...
// docdbSubnetGroupLister implements the DocdbSubnetGroupLister interface.
...
func NewDocdbSubnetGroupLister(indexer cache.Indexer) DocdbSubnetGroupLister {
	return &docdbSubnetGroupLister{indexer: indexer}
}

// List lists all DocdbSubnetGroups in the indexer.
func (s *docdbSubnetGroupLister) List(selector labels.Selector) (ret []*v1alpha1.DocdbSubnetGroup, err error) {
...
func (s docdbSubnetGroupNamespaceLister) Get(name string) (*v1alpha1.DocdbSubnetGroup, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("docdbsubnetgroup"), name)
	}
	return obj.(*v1alpha1.DocdbSubnetGroup), nil
}
