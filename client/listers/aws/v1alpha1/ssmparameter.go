...
// ssmParameterLister implements the SsmParameterLister interface.
...
func NewSsmParameterLister(indexer cache.Indexer) SsmParameterLister {
	return &ssmParameterLister{indexer: indexer}
}

// List lists all SsmParameters in the indexer.
func (s *ssmParameterLister) List(selector labels.Selector) (ret []*v1alpha1.SsmParameter, err error) {
...
func (s ssmParameterNamespaceLister) Get(name string) (*v1alpha1.SsmParameter, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("ssmparameter"), name)
	}
	return obj.(*v1alpha1.SsmParameter), nil
}
