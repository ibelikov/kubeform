...
// daxSubnetGroupLister implements the DaxSubnetGroupLister interface.
...
func NewDaxSubnetGroupLister(indexer cache.Indexer) DaxSubnetGroupLister {
	return &daxSubnetGroupLister{indexer: indexer}
}

// List lists all DaxSubnetGroups in the indexer.
func (s *daxSubnetGroupLister) List(selector labels.Selector) (ret []*v1alpha1.DaxSubnetGroup, err error) {
...
func (s daxSubnetGroupNamespaceLister) Get(name string) (*v1alpha1.DaxSubnetGroup, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("daxsubnetgroup"), name)
	}
	return obj.(*v1alpha1.DaxSubnetGroup), nil
}
