...
// gameliftBuildLister implements the GameliftBuildLister interface.
...
func NewGameliftBuildLister(indexer cache.Indexer) GameliftBuildLister {
	return &gameliftBuildLister{indexer: indexer}
}

// List lists all GameliftBuilds in the indexer.
func (s *gameliftBuildLister) List(selector labels.Selector) (ret []*v1alpha1.GameliftBuild, err error) {
...
func (s gameliftBuildNamespaceLister) Get(name string) (*v1alpha1.GameliftBuild, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("gameliftbuild"), name)
	}
	return obj.(*v1alpha1.GameliftBuild), nil
}
