...
// budgetsBudgetLister implements the BudgetsBudgetLister interface.
...
func NewBudgetsBudgetLister(indexer cache.Indexer) BudgetsBudgetLister {
	return &budgetsBudgetLister{indexer: indexer}
}

// List lists all BudgetsBudgets in the indexer.
func (s *budgetsBudgetLister) List(selector labels.Selector) (ret []*v1alpha1.BudgetsBudget, err error) {
...
func (s budgetsBudgetNamespaceLister) Get(name string) (*v1alpha1.BudgetsBudget, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("budgetsbudget"), name)
	}
	return obj.(*v1alpha1.BudgetsBudget), nil
}
