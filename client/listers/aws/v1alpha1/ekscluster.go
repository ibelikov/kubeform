/*
Copyright The Kubeform Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by lister-gen. DO NOT EDIT.

package v1alpha1

import (
	"k8s.io/apimachinery/pkg/api/errors"
	"k8s.io/apimachinery/pkg/labels"
	"k8s.io/client-go/tools/cache"
	v1alpha1 "kubeform.dev/kubeform/apis/aws/v1alpha1"
)

// EksClusterLister helps list EksClusters.
type EksClusterLister interface {
	// List lists all EksClusters in the indexer.
	List(selector labels.Selector) (ret []*v1alpha1.EksCluster, err error)
	// EksClusters returns an object that can list and get EksClusters.
	EksClusters(namespace string) EksClusterNamespaceLister
	EksClusterListerExpansion
}

// eksClusterLister implements the EksClusterLister interface.
type eksClusterLister struct {
	indexer cache.Indexer
}

// NewEksClusterLister returns a new EksClusterLister.
func NewEksClusterLister(indexer cache.Indexer) EksClusterLister {
	return &eksClusterLister{indexer: indexer}
}

// List lists all EksClusters in the indexer.
func (s *eksClusterLister) List(selector labels.Selector) (ret []*v1alpha1.EksCluster, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.EksCluster))
	})
	return ret, err
}

// EksClusters returns an object that can list and get EksClusters.
func (s *eksClusterLister) EksClusters(namespace string) EksClusterNamespaceLister {
	return eksClusterNamespaceLister{indexer: s.indexer, namespace: namespace}
}

// EksClusterNamespaceLister helps list and get EksClusters.
type EksClusterNamespaceLister interface {
	// List lists all EksClusters in the indexer for a given namespace.
	List(selector labels.Selector) (ret []*v1alpha1.EksCluster, err error)
	// Get retrieves the EksCluster from the indexer for a given namespace and name.
	Get(name string) (*v1alpha1.EksCluster, error)
	EksClusterNamespaceListerExpansion
}

// eksClusterNamespaceLister implements the EksClusterNamespaceLister
// interface.
type eksClusterNamespaceLister struct {
	indexer   cache.Indexer
	namespace string
}

// List lists all EksClusters in the indexer for a given namespace.
func (s eksClusterNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.EksCluster, err error) {
	err = cache.ListAllByNamespace(s.indexer, s.namespace, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.EksCluster))
	})
	return ret, err
}

// Get retrieves the EksCluster from the indexer for a given namespace and name.
func (s eksClusterNamespaceLister) Get(name string) (*v1alpha1.EksCluster, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("ekscluster"), name)
	}
	return obj.(*v1alpha1.EksCluster), nil
}
