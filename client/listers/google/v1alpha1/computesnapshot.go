...
// computeSnapshotLister implements the ComputeSnapshotLister interface.
...
func NewComputeSnapshotLister(indexer cache.Indexer) ComputeSnapshotLister {
	return &computeSnapshotLister{indexer: indexer}
}

// List lists all ComputeSnapshots in the indexer.
func (s *computeSnapshotLister) List(selector labels.Selector) (ret []*v1alpha1.ComputeSnapshot, err error) {
...
func (s computeSnapshotNamespaceLister) Get(name string) (*v1alpha1.ComputeSnapshot, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("computesnapshot"), name)
	}
	return obj.(*v1alpha1.ComputeSnapshot), nil
}
