/*
Copyright The Kubeform Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by lister-gen. DO NOT EDIT.

package v1alpha1

import (
	"k8s.io/apimachinery/pkg/api/errors"
	"k8s.io/apimachinery/pkg/labels"
	"k8s.io/client-go/tools/cache"
	v1alpha1 "kubeform.dev/kubeform/apis/google/v1alpha1"
)

// CloudfunctionsFunctionIamPolicyLister helps list CloudfunctionsFunctionIamPolicies.
type CloudfunctionsFunctionIamPolicyLister interface {
	// List lists all CloudfunctionsFunctionIamPolicies in the indexer.
	List(selector labels.Selector) (ret []*v1alpha1.CloudfunctionsFunctionIamPolicy, err error)
	// CloudfunctionsFunctionIamPolicies returns an object that can list and get CloudfunctionsFunctionIamPolicies.
	CloudfunctionsFunctionIamPolicies(namespace string) CloudfunctionsFunctionIamPolicyNamespaceLister
	CloudfunctionsFunctionIamPolicyListerExpansion
}

// cloudfunctionsFunctionIamPolicyLister implements the CloudfunctionsFunctionIamPolicyLister interface.
type cloudfunctionsFunctionIamPolicyLister struct {
	indexer cache.Indexer
}

// NewCloudfunctionsFunctionIamPolicyLister returns a new CloudfunctionsFunctionIamPolicyLister.
func NewCloudfunctionsFunctionIamPolicyLister(indexer cache.Indexer) CloudfunctionsFunctionIamPolicyLister {
	return &cloudfunctionsFunctionIamPolicyLister{indexer: indexer}
}

// List lists all CloudfunctionsFunctionIamPolicies in the indexer.
func (s *cloudfunctionsFunctionIamPolicyLister) List(selector labels.Selector) (ret []*v1alpha1.CloudfunctionsFunctionIamPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.CloudfunctionsFunctionIamPolicy))
	})
	return ret, err
}

// CloudfunctionsFunctionIamPolicies returns an object that can list and get CloudfunctionsFunctionIamPolicies.
func (s *cloudfunctionsFunctionIamPolicyLister) CloudfunctionsFunctionIamPolicies(namespace string) CloudfunctionsFunctionIamPolicyNamespaceLister {
	return cloudfunctionsFunctionIamPolicyNamespaceLister{indexer: s.indexer, namespace: namespace}
}

// CloudfunctionsFunctionIamPolicyNamespaceLister helps list and get CloudfunctionsFunctionIamPolicies.
type CloudfunctionsFunctionIamPolicyNamespaceLister interface {
	// List lists all CloudfunctionsFunctionIamPolicies in the indexer for a given namespace.
	List(selector labels.Selector) (ret []*v1alpha1.CloudfunctionsFunctionIamPolicy, err error)
	// Get retrieves the CloudfunctionsFunctionIamPolicy from the indexer for a given namespace and name.
	Get(name string) (*v1alpha1.CloudfunctionsFunctionIamPolicy, error)
	CloudfunctionsFunctionIamPolicyNamespaceListerExpansion
}

// cloudfunctionsFunctionIamPolicyNamespaceLister implements the CloudfunctionsFunctionIamPolicyNamespaceLister
// interface.
type cloudfunctionsFunctionIamPolicyNamespaceLister struct {
	indexer   cache.Indexer
	namespace string
}

// List lists all CloudfunctionsFunctionIamPolicies in the indexer for a given namespace.
func (s cloudfunctionsFunctionIamPolicyNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.CloudfunctionsFunctionIamPolicy, err error) {
	err = cache.ListAllByNamespace(s.indexer, s.namespace, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.CloudfunctionsFunctionIamPolicy))
	})
	return ret, err
}

// Get retrieves the CloudfunctionsFunctionIamPolicy from the indexer for a given namespace and name.
func (s cloudfunctionsFunctionIamPolicyNamespaceLister) Get(name string) (*v1alpha1.CloudfunctionsFunctionIamPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("cloudfunctionsfunctioniampolicy"), name)
	}
	return obj.(*v1alpha1.CloudfunctionsFunctionIamPolicy), nil
}
